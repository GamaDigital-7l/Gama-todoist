      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="flex items-center gap-3">
          {moodboard.thumbnail_url ? (
            <img src={moodboard.thumbnail_url} alt={moodboard.title} className="w-10 h-10 rounded-xl object-cover" />
          ) : (
            <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center text-muted-foreground text-lg font-semibold">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={() => onEdit(moodboard)} className="text-blue-500 hover:bg-blue-500/10 btn-glow">
            <Edit className="h-4 w-4" />
            <span className="sr-only">Editar Moodboard</span>
          </Button>
          <Button variant="ghost" size="icon" onClick={() => onDelete(moodboard.id)} className="text-red-500 hover:bg-red-500/10 btn-glow">
