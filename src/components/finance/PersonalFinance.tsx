...
const fetchPersonalGoals = async (userId: string): Promise<FinancialGoal[]> => {
  const { data, error } = await supabase
    ...
    `)
    .eq("user_id", userId)
    .in('type', ['personal_savings', 'purchase', 'travel']) // Filtrar por metas pessoais
...
const PersonalFinance: React.FC<PersonalFinanceProps> = ({ currentPeriod, onTransactionAdded }) => {
  ...
  const personalSavingsGoal = goals?.find(goal => goal.type === 'personal_savings');
  const personalSavingsProgress = personalSavingsGoal && personalSavingsGoal.target_amount > 0
    ? (personalSavingsGoal.current_amount / personalSavingsGoal.target_amount) * 100
    : 0;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Dialog open={isTransactionFormOpen} onOpenChange={setIsTransactionFormOpen}>
          ...
          <DialogContent
            ...
            aria-labelledby="personal-transaction-title"
            aria-describedby="personal-transaction-description"
          >
            <DialogHeader>
              <DialogTitle id="personal-transaction-title" className="text-foreground">
                {editingTransaction ? "Editar Transação Pessoal" : "Adicionar Transação Pessoal"}
              </DialogTitle>
              <DialogDescription id="personal-transaction-description" className="text-muted-foreground">
                {editingTransaction ? "Atualize os detalhes da transação pessoal." : "Registre uma nova receita ou despesa pessoal."}
              </DialogDescription>
            </DialogHeader>
            <TransactionForm
      ...
      {/* Cards de Resumo Específicos do Pessoal */}
        ...
        <Card className="bg-card border border-border rounded-xl shadow-sm frosted-glass card-hover-effect">
          ...
          <CardContent>
            {isLoadingGoals ? (
              ...
              <>
                <div className="text-2xl font-bold text-foreground">{formatCurrency(personalSavingsGoal?.current_amount || 0)}</div>
                {personalSavingsGoal && (
                  <>
                    <p className="text-xs text-muted-foreground">{personalSavingsProgress.toFixed(0)}% da meta de {formatCurrency(personalSavingsGoal.target_amount)}</p>
                    <Progress value={personalSavingsProgress} className="w-full mt-2" />
                  </>
                )}
                {!personalSavingsGoal && (
                  <p className="text-xs text-muted-foreground">Nenhuma meta de poupança pessoal definida.</p>
                )}
              </>
            )}
          </CardContent>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Dialog open={isRecurrenceFormOpen} onOpenChange={setIsRecurrenceFormOpen}>
          ...
          <DialogContent
            className={DIALOG_CONTENT_CLASSNAMES}
            aria-labelledby="personal-recurrence-title"
            aria-describedby="personal-recurrence-description"
          >
            <DialogHeader>
              <DialogTitle id="personal-recurrence-title" className="text-foreground">
                {editingRecurrence ? "Editar Recorrência Pessoal" : "Adicionar Recorrência Pessoal"}
              </DialogTitle>
              <DialogDescription id="personal-recurrence-description" className="text-muted-foreground">
                {editingRecurrence ? "Atualize os detalhes da recorrência." : "Registre uma nova receita ou despesa recorrente pessoal."}
              </DialogDescription>
            </DialogHeader>
            <RecurrenceForm
      ...
      {isLoadingRecurrences ? (
        ...
        <p className="text-muted-foreground">Nenhuma recorrência registrada para o pessoal.</p>
      )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Dialog open={isGoalFormOpen} onOpenChange={setIsGoalFormOpen}>
          ...
          <DialogContent
            className={DIALOG_CONTENT_CLASSNAMES}
            aria-labelledby="personal-goal-title"
            aria-describedby="personal-goal-description"
          >
            <DialogHeader>
              <DialogTitle id="personal-goal-title" className="text-foreground">
                {editingGoal ? "Editar Meta Financeira Pessoal" : "Adicionar Meta Financeira Pessoal"}
              </DialogTitle>
              <DialogDescription id="personal-goal-description" className="text-muted-foreground">
                {editingGoal ? "Atualize os detalhes da meta financeira." : "Crie uma nova meta financeira pessoal."}
              </DialogDescription>
            </DialogHeader>
            <FinancialGoalForm
              initialData={editingGoal ? { ...editingGoal, target_date: editingGoal.target_date ? parseISO(editingGoal.target_date) : null } : { type: "personal_savings" }}
      ...
      {isLoadingGoals ? (
        ...
        <p className="text-muted-foreground">Nenhuma meta financeira registrada para o pessoal.</p>
      )}

      <Separator className="my-6" />

      {/* Listagem de Transações Pessoais */}
      <Card className="bg-card border border-border rounded-xl shadow-sm frosted-glass card-hover-effect">
        <CardHeader>
