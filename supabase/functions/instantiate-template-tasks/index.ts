...
import { format, getDay, parseISO, isToday, setHours, setMinutes, isPast } from "https://esm.sh/date-fns@3.6.0"; // Versão atualizada
...
serve(async (req) => {
  ...
  try {
    ...
    for (const user of users || []) {
      ...
      const userTimezone = user.timezone || SAO_PAULO_TIMEZONE;
      ...
      if (tasksToInsert.length > 0) {
        const { data: newTasks, error: insertTasksError } = await supabase
          ...
          .select('id, user_id, title, recurrence_time');
        ...
        console.log(`[User ${userId}] Instanciadas ${newTasks?.length || 0} tarefas a partir de templates.`);
      } else {
